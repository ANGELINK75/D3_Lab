...
d3.csv('data/stacked_area2.csv').then((data) => {
    ...
    stack.keys(keys).order(d3.stackOrderNone).offset(d3.stackOffsetNone);

    // TODO: bind the data to the stack and create the group 
    // that will contain the area path
    var bindStack = g.selectAll('.browser').data(stack(data)).enter().append('g')
	            .attr('class', (d) => { return 'browser ' + d.key; }).attr('fill-opacity', 0.4);

    // TODO: call the area generator with the appropriate data
    bindStack.append('path').attr('class', 'area').attr('d', area).style('fill', (d) => { return color(d.key); });

    // Create legend
    // TODO: Create a legend showing all the names of every color
    var legend = g.append("g").attr("transform", "translate(" + (width + 150) + "," + (height - 210) + ")").attr("fill", "black");
    ...
    for (var i = 0; i < keysReversed.length; i++){
        ...
        line.append("rect").attr("width", 5).attr("height", 5).attr("fill", color(keysReversed[i]));
        line.append("text").attr("x", -10).attr("y", 10).attr("text-anchor", "end").text(keysReversed[i]);
    }
